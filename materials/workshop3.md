...
graph TD
    ...
    SW --> |Тесты на утверждение| H
    H --> |Утверждено| CA[📋 Contract Agent]

    CA --> |API контракты| H
    H --> |Утверждено| IMPL[⚙️ Implementation Agents]
...
- **Человек утверждает**: тесты и контракты (критические точки контроля)
- **Параллельная разработка**: Backend, Frontend, Data работают одновременно после утверждения контрактов
- **Циклы обратной связи**: Mutator находит проблемы → Implementation исправляет
...
sequenceDiagram
    ...
    DE->>H: Уточняющие вопросы
    H->>DE: Ответы
    DE->>SW: Передача инсайтов

    SW->>SW: Написание BDD-тестов
    SW->>H: Тесты на утверждение ⚠️
    H->>SW: Утверждено ✅

    SW->>CA: Запрос на контракты
    CA->>CA: Проектирование API
    CA->>H: Контракты на утверждение ⚠️
    H->>CA: Утверждено ✅

    par Параллельная разработка
        CA->>BE: Реализация Backend
        CA->>FE: Реализация Frontend
    end
    ...
    REF->>H: Готово к продакшну 🚀
```

**Ключевые моменты процесса:**

1. **Интервью:** `Domain Expert` и `Product Analyst` общаются с заказчиком.
2. **Написание тестов:** `Spec Writer` создает полный набор сценариев.
3. **Утверждение (Человек):** Человек проверяет и утверждает тесты. **Это самый важный этап.**
4. **Проектирование контрактов:** `Contract Agent` определяет API. Человек утверждает контракты. **Опять человек.**
5. **Параллельная разработка:** Backend-агент пишет логику для сохранения данных в базу, а Frontend-агент в это же время создает форму для ввода этих данных, доверяя, что контракт API будет соблюден. Это ускоряет процесс в разы.
6. **Тестирование и отладка:** `Mutator` ищет уязвимости, а система собирается воедино и прогоняется через все тесты.
7. **Рефакторинг:** `Refactoring Agent` оптимизирует решение.
8. **Итерации:** Для добавления новой функциональности цикл повторяется.

**Точки эскалации (когда система ждет человека):**

- **Утверждение acceptance-тестов и контрактов.** Пример: AI: "Вот тесты для функции регистрации. Убедитесь, что мы учли все сценарии". Человек: "Отлично, но добавьте тест на случай, если пользователь вводит слишком короткий пароль".
- **Разрешение противоречий в требованиях.** Пример: AI: "Тест А требует, чтобы доступ был у всех, Тест Б — только у платных пользователей. Какое правило верное?".
- **Принятие ключевых архитектурных решений.** Пример: AI: "Мы можем построить систему как единое целое (монолит) или как набор независимых сервисов (микросервисы). Первый вариант быстрее в начале, второй — гибче в будущем. Ваш выбор?".
- **Системные сбои интеграции.** Пример: AI: "Backend-агент и Frontend-агент трижды не смогли синхронизировать данные по контракту. Вероятно, контракт нужно пересмотреть. Как поступим?".
- **Превышение бюджетных лимитов на API.** Пример: AI: "Мы потратили 80% месячного бюджета на API. Продолжаем разработку или ставим на паузу?".
