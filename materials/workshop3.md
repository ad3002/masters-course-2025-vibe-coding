...
```mermaid
graph LR
    subgraph "🔴 Традиционная разработка"
    ...
    subgraph "🟢 AI-агенты + Спецификации"
        A1[Идея] --> A2[Пишем тесты/спецификацию]
        A2 --> A3{Тесты утверждены?}
        A3 -->|Нет| A2
        A3 -->|Да| A4[AI генерирует код]
        A4 --> A5[Тесты проходят?]
        A5 -->|Нет| A4
        A5 -->|Да| A6[Деплой]
...
| Этап | Традиционная | AI-агенты | Ускорение |
|------|--------------|-----------|-----------|
...
**Ключевые моменты:**
- 👤 **Красные блоки** — требуют участия человека
- 🤖 **Зелёные блоки** — выполняются AI автономно
- ⚡ **Параллельная работа** — Backend и Frontend одновременно после утверждения контрактов
- 🎯 **Критический путь** — через утверждение тестов и контрактов человеком
...
| Задача | 👤 Человек | 🎯 Manager | 🤖 Агенты | Пояснение |
|--------|-----------|-----------|----------|-----------|
...
```mermaid
graph TB
    subgraph "💰 Стоимость проекта (веб-приложение средней сложности)"
        direction LR

        subgraph "🔴 Традиционная разработка"
            T_TIME["⏱️ Время<br/>168 часов<br/>(3 недели)"]
            T_COST["💵 Стоимость<br/>$8,400<br/>($50/час × 168ч)"]
            T_RISK["⚠️ Риски<br/>Высокие<br/>(багги после релиза)"]
        end

        subgraph "🟢 AI-агенты"
            A_TIME["⏱️ Время человека<br/>5 часов<br/>(1 день)"]
            A_TOKENS["🎫 Токены AI<br/>~$150-300<br/>(зависит от сложности)"]
            A_COST["💵 Общая стоимость<br/>~$500<br/>($50/ч × 5ч + токены)"]
            A_RISK["✅ Риски<br/>Низкие<br/>(тесты покрывают всё)"]
        end
    end

    T_COST -->|"В 17х дороже"| A_COST
    T_TIME -->|"В 34х дольше"| A_TIME
    T_RISK -->|"Выше риск"| A_RISK
